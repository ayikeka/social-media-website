...
class Post{
	...
	public function submitPost($body, $user_to, $imageName) {
		$body = strip_tags($body); //removes html tags from the body
		...
		if($check_empty != ""){
			...
			$body_array =preg_split("/\s+/", $body); //Reg-S is used for spliting strings and saving as individual strings
			...
			$stopWords = "a about above across after again against all almost alone along already
             ...
             $no_punctuation = preg_replace("/[^a-zA-Z 0-9]+/", "", $body); //Means it should replace anything that is not a number or letter.
		...
		if(mysqli_num_rows($data_query) > 0){
			...
			while($row = mysqli_fetch_array($data_query)){
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					//Keeps counting posts to make sure they reach number of posts per page view.
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<button class='delete_button' id='post$id' title='delete'>&times;</button>";
					else
						$delete_button="";
					...
					$interval = $start_date->diff($end_date); //Difference between date and time
					if($interval->y >= 1) {
						if($interval == 1)
							$time_message = $interval->y . " a year ago "; // 1 year ago
						else 
					...
					if ($imagePath != "") {
							$imageDiv = "<div class='postedImage'>
											<img src='$imagePath' id='Profile_Image'>
										</div>";
							}	
							else {
								$imageDiv = "";
					...
					$str .= "<div class='status_post' >
								...
								<div class='posted_by' style='color:#ACACAC;'>
									<a href='$added_by' class='fNlname'> $first_name $last_name </a> $user_to &emsp;&emsp; $time_message
									$delete_button
								</div>
								<div class='post_body'> $body <br> $imageDiv <br></div>

								<div class='newsfeedPostOptions' onClick='javascript:toggle$id()'>
									Comments($comment_check_num)  &emsp;&emsp;
			...
			if($count > $limit)
				...
				$str .= "<input type='hidden' class='noMorePosts' value='true'>
						<p style='text-align: center;'> <em>No more posts to show!</em></p>";
		}
		echo $str;
	}	
	...
	public function loadProfilePosts($data, $limit){
		...
		if(mysqli_num_rows($data_query) > 0){
			...
			while($row = mysqli_fetch_array($data_query)){
				...
				$imagePath = $row['image'];
					...
					$str .= "<div class='status_post' >
								...
								<div class='posted_by' style='color:#ACACAC;'>
									<a href='$added_by' class='fNlname'> $first_name $last_name </a> &emsp;&emsp; $time_message
									$delete_button
								</div>
								<div class='post_body'> $body <br> $imageDiv <br> </div>
			...
			if($count > $limit)
				...
				$str .= "<input type='hidden' class='noMorePosts' value='true'>
						<p style='text-align: center; margin:0;'> <em>No more posts to show!</em></p>";
		}
		echo $str;
	}	
		...
		if(mysqli_num_rows($data_query) > 0){
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					$str .= "<div class='status_post' >
								...
								<div class='post_body'> $body <br> </div>
				...
				echo "<p>You cannot see this post because you're not friends with this user.</p>";
				return;
